...
# You are given an array and you need to find number of tripets of indices (i, j, k) such that the elements at those indices are
# in geometric progression for a given common ratio  and i < j < k


def countTriplets(arr, r):
    l = [(n, r * n, r * r * n) for n in arr if r * n in arr and r * r * n in arr]
    s = set(l)
    return len(s)
